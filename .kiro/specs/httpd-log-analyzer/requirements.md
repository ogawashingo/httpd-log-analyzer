...
この機能は、Apache/NginxのHTTPサーバーログ（access_logとerror_log）を解析し、疑わしいアクセスパターンを特定・抽出するシェルスクリプトの作成を含みます。このツールは、ブルートフォース攻撃、SQLインジェクション攻撃、存在しないページへのアクセス、高頻度アクセスパターン、WAFによってブロックされた攻撃、権限昇格の試行など、様々な種類の悪意のある活動を検出します。access_logとerror_logの両方を解析することで、より包括的なセキュリティ監視を実現します。出力には、送信元IPアドレス、発生回数、疑わしいとフラグ付けされた理由、地理的位置情報が含まれます。
...
**ユーザーストーリー:** システム管理者として、SQLインジェクション攻撃の試行を検出したい。これにより、潜在的なデータベース侵害の試みを特定し、ブロックすることができる。

#### 受け入れ基準

1. リクエストURLとパラメータを解析する時、システムは一般的なSQLインジェクションパターンを検索する
2. SQLインジェクションパターンを検出する時、システムは「union」、「select」、「drop」、「insert」、「update」、「delete」、「script」、「alert」、「onload」、「javascript」、「exec」、「information_schema」、「benchmark」、「sleep」などのキーワードを特定する
3. SQLインジェクションパターンが見つかる場合、システムは送信元IPを「SQLインジェクション攻撃の可能性」の理由で疑わしいとフラグ付けする
4. URLエンコードされたSQLパターンが存在する時、システムはそれらをデコードして解析する
